...
{
    if (Object.values(req.body).length!=5 || !req.body.ra || !req.body.long || !req.body.lat || !req.body.data || !req.body.imagem )
    {
        const erro = Comunicado.novo('DdI','Dados inesperados','Não foram fornecidos exatamente as 5 informaçães esperadas de um foto (ra, long, lat e preço)').object;
    ...
    try
    {
        foto = Foto.novo (req.body.ra,req.body.long,req.body.lat || req.body.data ||req.body.imagem );
    }
    catch (excecao)
    {
        const erro = Comunicado.novo('TDE','Dados de tipos errados','Codigo deve ser um numero natural positivo, long deve ser um texto não vazio e preço deve ser um número real positivo').object;
        return res.status(422).json(erro);
    }
    ...
    if (ret===false)
    {
        const  erro = Comunicado.novo('LJE','Foto já existe','Já há foto cadastrado com o código informado').object;
...
async function atualizacao (req, res)
{
    if (Object.values(req.body).length!=5 || !req.body.ra || !req.body.long || !req.body.lat || !req.body.data || !req.body.imagem)
    {
        const erro = Comunicado.novo('DdI','Dados inesperados','Não foram fornecidos exatamente as 5 informações esperadas de um foto (ra atual, novo long e novo preço)').object;
        return res.status(422).json(erro);
    }
    ...
    try
    {
        foto = Foto.novo (req.body.ra,req.body.long,req.body.lat || long,req.body.data || long,req.body.imagem );
    }
    catch (excecao)
    {
        const erro = Comunicado.novo('TDE','Dados de tipos errados','Codigo deve ser um numero natural positivo, long deve ser um texto não vazio e preço deve ser um número real positivo').object;
